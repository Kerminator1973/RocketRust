...
На Rust был переписан **репозитарий npm** и DNS-сервера от Cloudflare. Отмечается, что Rust-приложения обладают отличной предсказуемостью операций выделения памяти, чем не могут похвастаться приложения на Go, Java, JavaScript и платформы .NET. Также на Rust написан движок Quantum (Mozilla Firefox).
