...
Синтаксически, Rust требует максимальной определённости конструкций, например, не допуская смешения типов в арифметических операциях. Преобразования типов не могут выполняться неявно - если есть смешение типов, компилятор потребует устранить неднозначность вручную.
