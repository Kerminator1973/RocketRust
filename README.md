...
Уникальная "фича" Rust - **Borrow checker**, контроллер заимствования, основная идея которого: данные в программе не могут "висеть в воздухе", они всегда должны иметь владельца. переменные, объявленные внутри функции, принадлежат области видимости этой функции. Это и есть **владение**. Рекомендуется к прочтению статья [Почему в Rust всё правильно](https://habr.com/ru/companies/domclick/articles/970010/) by evgeniyrru.
...
- Figma разрабатывает Frontend на Rust, с трансляцией в WebAssembly. Также они перенесли на Rust и свой Backend
...
Разработчики Ubuntu в версии 25.10 замещают основные утилиты (uutils project/coreutils) реализацией на Rust. Попробовать coreutils, написанные на Rust можно уже сейчас, используя проект [oxidizr](https://github.com/jnsgruk/oxidizr). Для некоторых утилит время выполнения сократилось в шесть раз! Причина высокой производительности - удачная реализации многопоточности, в частности, при работе с текстами (grep, regex).

>Ноябрь 2025 года: реализация coreutils на Rust оказалась полным провалом. Продукт не зрелый, содержит множество уязвимостей и Canonical продлила LTS (версия Ubuntu без coreutils на Rust) на 10 лет. Однако [поддержка Rust переведена из экспериментальных в основные возможности ядра Linux](https://www.opennet.ru/opennews/art.shtml?num=64401).
>
>В стандерте C++ 23 появилась конструкция `std::expected<>`, которая позволяет эффективно возвращать как признак успешности выполнения функции/метода, так и фактический результат. Т.е. другие языки программирования активно заимствуют наиболее удачные решения Rust.
>
>Стали появляться т.н. "безопасные компиляторы C++", которые обеспечивают встроенную защиту от типовых ошибок программирования на C++, максимально приближаясь к уровню защиты Rust. Цель этих проектов состоит именно в том, чтобы сделать C++ не менее защищённым, чем Rust.
>
>Microsoft активно развивает Blazor как основной Framework для разработки web-приложений, компилиуемых в WebAssembly.

>Декабрь 2025 года: в статье [Статистика по языкам программирования, используемым в экосистеме GNOME](https://blogs.gnome.org/sophieh/2025/12/27/gnome-in-2025-some-numbers/) приведена информация о том, что в GNOME 6 692 516 строк кода, из которых 5 млн. - это библиотеки и вспомогательные компоненты, а 1.6 млн. - приложения. На Rust написано 10.3% из GNOME Core Apps и 41.7% GNOME Circle Apps.

В Linux-дистрибутиве Pop!OS на Rust переписан _Desktop Environment_ - **Cosmic DE**, который полностью заменяет Gnome.
...
- Cloudflare заменил ключевые системы, требующие высокой производительности, на Rust (26 сентября 2025 г). Однако через 53 дня (18 ноября 2025 г) у Cloudflare происходит масштабный сбой, который вывел из строя значительные части Интернета, из-за ошибки... в том самом Rust-коде. Рекомендуется для просмотра видео "The line of code that took down the Internet" by **ThePrimeTimeagen**. Проблема была в некорректном вызове **unwrap**(), в модуле, который борется с Bot-ами посредством постоянно изменяемых правил идентификации bot-ов (Bot management). В ПО использовалось около 60 правил (features), которые обновляются вручную каждый 5 минут. Привила работают на прокси и ПО ограничивает количество применяемых правил 200-ми. По каким-то причина были опубликованы более 200 правил и это сломало приложение на Rust, которое вызвало _program panic_ на прокси-сервере и именно это привело к проблемам в работе Интернет в течение дня. Это хорошая иллюстрация того, что язык сам по себе не избавляет от проблем, а лишь помогает (или нет) использовать процессе, обеспечивающие надёжную и эффективную работу промышленных сервисов
...
Сравнивая производительность Rust и C++, можно говорить о паритете, особенно с учётом использования обоими языками [LLVM](https://llvm.org/), в качестве оптимизирующего компилятора из промежуточного представления в нативный код.
...
>
>Ноябрь 2025: в Python 3.17 планируется сделать Rust обязательным для инфраструктуры сборки CPython. Текущие версии CPython написаны на Си, но постепенно появляется всё больше компонентов, написанных на Rust.
>
>Схожая картина с разработкой операционной системы Android - появляются драйвера устройств написанные на Rust, а также отдельные сервисы. Благодаря возможности симбиоза C++ и Rust, не требуется революционных изменений - эволюция работает вполне хорошо.
...
6) В Runtime широко поддержан шаблон проектирования _chaining methods_

```rs
for i in number.trim().chars() {
```

7) Атрибутированный код может быть использован для анализа исходного текста и содержимого файла "Cargo.toml" для генерации контента на этапе компиляции. Так, например, библиотека [clap](https://docs.rs/clap/latest/clap/) (разбор командной строки) извлекает комментарии и данные из "Cargo.toml" для генерации справочного текста об использовании приложения

8) Использование **inline asm**  позволяет создавать супер-эффективные (в контексте расхода вычислительных ресурсов) приложения:
...
9) Встроенные в компилятор проверки кода заставляют программистов сразу подумать о хорошем коде. В других языках программирования, часто реализуется другой подход: сначала написать как-нибудь, потом улучшать, а когда-нибудь в отдалённом будущем начать использовать анализаторы кода, проверяльщики выхода за пределы массива, искатели нарушения в многопоточной модели, и т.д.
...
В Rust существует тип String - это строка с владением (_ownership_). Мы можем создать ссылку на строку, или на её часть. Ссылка на строку является другим типом - `&str`. Получение ссылки на строку называется заимствованием (_borrowing_). Ключевая особенность Rust состоит в том, что строка не может быть освобождена до тех пор, пока существуют ссылки на неё, т.е. Rust автоматически отслеживает доступность "внешней" памяти пот использовании ссылки, или, как это называется в Rust - _slice_. Использование ссылок/слайсов позволяет избежать копирование элементов в памяти и таким образом, кратно повышать эффективность работы приложения. Ссылка хранит указатель на начало slice в строке и её длину. Следует заметить, что типы String и &str не совместимы между собой и требуют преведения. Например, можно создать новую строку (String) из str используя функцию `to_string()`. Функции могут возвращать только строки типа String.

Пример создания slice из String:
...
Собрать слайс из строки можно используя функцию chars():

```rs
let slice = my_string.chars().take(5).collect(); 
```
...
- [Axum](https://github.com/tokio-rs/axum) (Rust) обрабатывает в 1,5 больше запросов, чем [Drogon](https://drogon.org/) (C++). Это косвенная характеристика
