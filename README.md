...
4) Использование макросов print!(), println!() и format!() для реализации аналога variadic-ов. Макросы могут использовать исходные тексты и "Cargo.toml" для генерации контента на этапе компиляции. Так, например, библиотека [clap](https://docs.rs/clap/latest/clap/) (разбор командной строки) извлекает комментарии и данные из "Cargo.toml" для генерации справочного текста об использовании приложения.
