...
Может возникнуть ситуация, когда обработка Ok()/Err() является явно избыточной. В этом случае мы можем к переменным типа `Option`, или `Result` применить функцию `unwrap()`, которая либо вернёт значение варианта Ok(), или вызовет `panic!()`. Примеры:

```rs
let maybe_value: Option<i32> = Some(42);
let val = maybe_value.unwrap(); // вернёт 42

let no_value: Option<i32> = None;
// Следующий код вызовет panic!():
// let val = no_value.unwrap();
```

"Под капотом", unwrap() это просто shortcut для match:
...
Важно заметить, что сигнатура `&mut` относится не к ссылке, а к типу, т.е. ссылку мы менять не можем, но может изменять значение, на которое указывает ссылка.

Если мы хотим, чтобы изменяемой была ссылка, то следует добавить модификатор mut перед именем ссылки:

```rs
let mut val_a = 5;
let mut val_b = 10;

let mut reference: &i32 = &val_a;
reference = &val_b;
```

Чтобы сделать изменяемыми и ссылку и значение по ссылке, определение должно выглядеть следующим образом:
...
Если на значение уже есть изменяемая ссылка, то других изменяемых ссылок на это же значение быть не должно.

В Rust существует механизм _reborrowing_, который позволяет создать новую ссылку на уже заимствованные данные, т.е. временно заимствовать данные даже при существующем заимствовании.

Также в Rust существует конструкция явного связывания жизненного цикла переменных, что позволяет добиться успешной компиляции приложения, в случая, если у компилятора есть сомнения о корректности жизненного цикла переменных. Эта конструкция называется _explicit lifetime annotation_. Сигнатура метода с её использованием выглядит как: `foo<'a, 'b>`. Пример заголовка функции:
...
Константы в Rust требуют аннотации типа:

```rs
const DEGREE: i32 = 25;
```

Константы могут содержать вычисления, но без использования переменных:

```rs
const DEGREE: i32 = 2 * 14 * 10;
```

## Затенение

В Rust возможно объявить новую переменную с тем же именем, как и у существующей переменной:
...
Во второй строке будет создана новая переменная, которая заменит собой определённую в первой строке. Этот инструмент называется "затенение".
